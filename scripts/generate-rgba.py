...
def create_geometric_image(width, height):
    ...
    for x in range(50, min(250, width)):
        for y in range(50, min(150, height)):
            image.putpixel((x, y), (255, 0, 0, 255))  # Set alpha to 255
...
def main():
    ...
    # Save the image in raw RGBA format
    with open("/tmp/geometric_image.rgba", "wb") as f:
        f.write(image.tobytes())
    logging.info("Image saved as raw RGB format: /tmp/geometric_image.rgba")
